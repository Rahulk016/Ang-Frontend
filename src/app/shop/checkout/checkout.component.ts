...
export class CheckoutComponent implements OnInit {
  ...
  constructor(public productsService: ProductsService, private _dataService: HttpService, private _fb: FormBuilder, 
    private _toastr: ToastrService, private cartService: CartService, private orderService: OrdersService) { }
  ...
  onSubmit(formData: any) {
    ...
    this._dataService.post(environment.BASE_API_PATH + "PaymentMaster/Save/", obj).subscribe(objdata => {
      if (objdata.isSuccess) {
        ...
        this.orderService.setOrderDetils(orderDts);
  ...
  var handler = (<any>window).StripeCheckout.configure({
    ...
    token: function (token: any) {
      let objPayment = { tokenId: token.id, amount: obj.amount + obj.shippingAmount, description: "Shopping with Amazon mall - Ajeet Singh - 13-Oct" };
      //console.log(token);
